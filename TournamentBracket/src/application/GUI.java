...
public class GUI {
	 ...
	 * 
	 * the bottom contains a picture
	 * the left contains a VBox with a load button and a list of teams
	 * the center contains a huge hBox, with a list of VBox
	 * each VBox contains number of games according to the VBox index (column number) 
	 * 
	 * @param root
	 * @return BorderPane
	 */
    ...
    /**
     * check number of teams within the file
     * if there's no team, directly pop out an alert box 
     * if there's only one team, pop out an alert box saying the specified team is 
     * the winner
     * if there's more than 1 team, call setupCenterRegular() method
     * 
     * @return HBox
     */
    private static HBox setupCenter() {
        ...
        if (Main.teams.size() > 1) {
            ...
            if (Main.teams.size() == 0) {
                ...
                teamLabel.setFont(new Font("Algerian", 13));
    ...
    /**
     * set up the center of BorderPane with a list of VBox
     * each VBox contains a specific number of games according to its
     * VBox index number within VBoxList (or its column number)
     * 
     * @return HBox
     */
    private static HBox setupCenterRegular() {
        ...
        vBoxList.get(vBoxNum / 2).getChildren()
        		.add(Main.games.get(Main.games.size() - 1).setUpVBox());
        ...
        for (int i = 0; i < vBoxNum / 2; i++) {
            ...
            for (int j = 0; j < (int) Math.pow(2, vBoxNum / 2 - 1 - i); j++) {
                vBoxList.get(i).getChildren().add(Main.games.get(index).setUpVBox());
                index++;
            }
            // this loop inserts from the right side of vBoxList
            for (int j = 0; j < (int) Math.pow(2, vBoxNum / 2 - 1 - i); j++) {
                vBoxList.get(vBoxList.size() - i - 1).getChildren()
                                .add(Main.games.get(index).setUpVBox());
                index++;
            }
    ...
    /**
     * set up the left side of BorderPane
     * which contains a load button and a list of teams
     * 
     * @return VBox
     */
     ...
     * loads the new file with the new team list into GUI
     * also, clear the previous team list within GUI
     * 
     * @return Button
     */
    ...
    /**
     * set up the bottom of BorderPane with a picture
     * set this picture to right by adding another BorderPane 
     * 
     * @return BorderPane
     */
    private static BorderPane setupBottom() {
        Image image = new Image(
                        "file:///" + new File("march-madness.png").getAbsolutePath().toString(),
                        350,
                        350, true, true);
        ImageView imageView = new ImageView(image);
        BorderPane bottom = new BorderPane();
        bottom.setRight(imageView);
        return bottom;
    }
}
