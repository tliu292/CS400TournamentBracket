...
/**
 * Game class stores information of each particular game each game contains a gameNumber,
 * differentiating one game from another and also two teams compete when after clicking the submit
 * button
 */
public class Game {
    ...
    private int team1;
    private int team2;
    private VBox vBox;// the Vbox that contains two teams and one button
    private boolean isTeam1Wins;// judge if team1 wins or loses to team 2

    /**
     * Constructor of a Game object
     * 
     * @param number counts the order of games.
     * @param team1 is the rank of the first team in this game.
     * @param team2 is the rank of the second team in this game.
     * @param vBox is container of two hboxes and one button representing a game.
     */
     ...
     * Get if team1 wins
     * 
     * @return a boolean to indicate if team 1 wins or loses
     */
    ...
    /**
     * Set which team 1 wins or loses
     * 
     * @param isTeam1Wins indicates if team 1 wins or loses
     */
    ...
    /**
     * Get the ordinal number of the game 
     * 
     * @return the ordinal number of the game 
     */
    ...
    /**
     * Set the ordinal number of the game
     * 
     * @param number is the ordinal number of the game
     */
    ...
    /**
     * Get team 1's original ordinal number
     * 
     * @return team 1's original ordinal number
     */
    public int getTeam1() {
        return team1;
    }
    ...
    /**
     * Setup team 1's original ordinal number
     * 
     * @param team1 is original ordinal number of team 1
     */
    ...
    /**
     * Get team 2's original ordinal number
     * 
     * @return team 2's original ordinal number
     */
    public int getTeam2() {
        return team2;
    }
    ...
    /**
     * Setup team 2's original ordinal number
     * 
     * @param team2 is original ordinal number of team 2
     */
    ...
    /**
     * Get the Vbox representing a game, containing Hboxes and a button
     * 
     * @return a Vbox representing a game, containing Hboxes and a button
     */
    public VBox getVBox() {
        return vBox;
    }
    ...
    /**
     * Set up a Vbox representing a game, containing two Hboxes and a button
     * 
     * @param vBox represents a game and contains two Hboxes and a button
     */
    ...
    /**
     * sets up the VBox with two team HBox and one submit button the submit button functionality
     * changes if there's only one game left in the tournament
     * 
     * @return VBox
     */
    public VBox setUpVBox() {
        ...
        submit.setOnMouseEntered(e -> {
            submit.setStyle("-fx-text-fill: #f7f7f7;");
        });
        submit.setOnMouseExited(e -> {
            submit.setStyle("-fx-text-fill: #c5050c;");
        });
        ...
        if (getNumber() == Main.teams.size() - 1) {
            /*
             * this method is called if there's only one game within the tournament pop out an alert
             * box if the two teams have the same score otherwise, pop out an information box with
             * the top three winners
             */

            submit.setOnAction(e -> {
                try {
                    Main.teams.get(team1).setScore(Integer.parseInt(
                                    ((TextField) team1HBox.getChildren().get(1)).getText()));
                    Main.teams.get(team2).setScore(Integer.parseInt(
                                    ((TextField) team2HBox.getChildren().get(1)).getText()));
                } catch (Exception ex) {
                ...
                if (score == 0) {
                    ...
                    alert.setContentText(
                                    "Please let those two teams have a rematch and then retype their scores.");
                ...
                String temp1 = ((TextField) ((HBox) vBox.getChildren().get(0)).getChildren().get(1))
                                .getText();
                ...
                String temp2 = ((TextField) ((HBox) vBox.getChildren().get(4)).getChildren().get(1))
                                .getText();
                ...
                if (getNumber() == 1) {
                    ...
                    int score2 = Main.teams.get(team3).getScore()
                                    - Main.teams.get(team4).getScore();
                    int third = team3;
                    if (score2 < 0)
                        third = team4;
                    if (score2 == 0)
                    ...
                    info.setContentText("The following are the top three teams:\n" + "1st -> "
                                    + Main.teams.get(winner).getName() + "\n2nd -> "
                                    + Main.teams.get(loser).getName() + "\n3rd -> " + (third == -1
                                                    ? Main.teams.get(team3).getName() + " and "
                                                                    + Main.teams.get(team4)
                                                                                    .getName()
            ...
            /*
             * this is how a regular button functions or when there's more than 1 team in the
             * tournament proceed to the next round if 1 team has higher score than the other pop
             * out a confirmation dialog when two teams have the same score
             */
            submit.setOnAction(e -> {
                try {
                    Main.teams.get(team1).setScore(Integer.parseInt(
                ...
                if (score > 0) {
                    ...
                    String temp1 = ((TextField) ((HBox) vBox.getChildren().get(0)).getChildren()
                                    .get(1)).getText();
                    Label scoreLabel1 = new Label(temp1);
                    scoreLabel1.getStyleClass().add("score-label");
                    ((HBox) vBox.getChildren().get(0)).getChildren().remove(1);
                    ...
                    String temp2 = ((TextField) ((HBox) vBox.getChildren().get(4)).getChildren()
                                    .get(1)).getText();
                    Label scoreLabel2 = new Label(temp2);
                    scoreLabel2.getStyleClass().add("score-label");
                    ((HBox) vBox.getChildren().get(4)).getChildren().remove(1);
    ...
    /**
     * update the team to the next round by refreshing the label with the new team's ranking and its
     * name
     * 
     * @param i
     * @param team
